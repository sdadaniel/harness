...
export function getHarnessConfiguration(
  ...
  const cfg = vscode.workspace.getConfiguration("hSync");
...
Run **HSync: Sync Skills & Agents** to install into Cursor, Claude Code, and Codex.
`,
      "utf8"
    );
  }
...
export function onConfigChange(
  ...
  return vscode.workspace.onDidChangeConfiguration((e) => {
    if (e.affectsConfiguration("hSync")) {
      listener();
    }
  });
}
